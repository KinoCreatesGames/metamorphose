 ...
 */
class FadeToWhite extends dn.Process {
  ...
  public function new() {
    ...
    mask = new h2d.Bitmap(h2d.Tile.fromColor(0xffffff, 1, 1, 1), root);
    mask.alpha = 0;

    root.under(mask);

    transitionTween = new Tweenie(Const.FPS);
    var tween = transitionTween.createS(mask.alpha, 1, TEase, TRANSITION_TIME);
